...
--------------------------------------------------------------------------------------------------------------*/
        ...
        public function GestionarReservas(){    
            ...
            $usuario = $claseUser->getUsuarioPorNombre($_SESSION['usuario']);
            $usuarioPerfil = new UsuarioPerfil();
            $perfilUsuario = $usuarioPerfil->getPerfilUsuarioPorIdUsuario($usuario->idUsuario);
            $reserva = new Reserva();
            $datosReserva = $reserva->ConsultarReservasClientePorEstado(1,intval($perfilUsuario->idPerfil));//1 para las reservas creadas
            require_once "view/dash/sidebarMenu.php";            
